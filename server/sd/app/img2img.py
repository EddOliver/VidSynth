...
def processing(path, index):
    global model
    global modelFS
    global modelCS
    assert os.path.isfile(path)
    init_image = load_img(path, opt.H, opt.W).to(opt.device)
            ...
            for prompts in tqdm(data, desc="data"):
                with precision_scope("cuda"):
                    ...
                    for i in range(batch_size):
                        ...
                        Image.fromarray(x_sample.astype(np.uint8)).save(f"{opt.outdir}/{index}.{opt.format}")
                        seeds += str(opt.seed) + ","
                        opt.seed += opt.increase
...
parser.add_argument("--init-path", type=str, nargs="?", help="path to the input images")
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Slightly increase the seed by a number",
)
...
myDir = sorted(os.listdir(opt.init_path),key=lambda f: int(''.join(filter(str.isdigit, f))))

for index,file in enumerate(myDir):
    processing(opt.init_path +"/"+ file,  index)
    print(file)
